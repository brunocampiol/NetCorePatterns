...
namespace NetCorePatterns.Singleton
{
    ...
    public sealed class SingletonSealedDisposableClass : IDisposable
    {
        ...
        private static volatile SingletonSealedDisposableClass _instance;
        private static readonly object _syncLock = new object();

        // No one but itself can instanciate
        private SingletonSealedDisposableClass()
        {
        }

        //uses a pattern known as double check locking
        public static SingletonSealedDisposableClass Instance
        {
            get
            {
                ...
                lock (_syncLock)
                {
                    if (_instance == null)
                    {
                        _instance = new SingletonSealedDisposableClass();
                    }
                }
                return _instance;
            }
